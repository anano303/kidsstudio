...
export class UsersService {
  ...
  async updateProfileImage(
    ...
    try {
      ...
      if (user.profileImagePath) {
        try {
          await this.cloudinaryService.deleteImageByFileId(
      ...
      // Get image URL
      const imageUrl =
        await this.cloudinaryService.getImageByFileId(profileImagePath);
  ...
  async uploadImage(filePath: string, fileBuffer: Buffer): Promise<string> {
    try {
      // Check if it's a logo path
      if (filePath.includes('logos/')) {
        return await this.cloudinaryService.uploadLogo(filePath, fileBuffer);
      }
  ...
  async getProfileData(userId: string) {
    ...
    if (user.profileImagePath) {
      profileImage = await this.cloudinaryService.getImageByFileId(
        user.profileImagePath as string,
      );
    }
  ...
  async getProfileImageUrl(profileImagePath: string): Promise<string | null> {
    ...
    try {
      return await this.cloudinaryService.getImageByFileId(profileImagePath);
    } catch (error) {
  ...
  private generateTokens(user: UserDocument) {
    const payload = { sub: user._id, email: user.email, type: 'access' };
